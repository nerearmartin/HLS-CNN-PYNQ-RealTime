...
__attribute__((sdx_kernel("cnn_stream_accel", 0))) void cnn_stream_accel(hls::stream<axis_t> &in_stream, hls::stream<axis_t> &out_stream) {
#pragma HLS TOP name=cnn_stream_accel
# 16 "wrapper.cpp"
...
#pragma HLS INTERFACE s_axilite port=return bundle=CTRL
 ...
 float img_buffer[28][28];
    ...
    VITIS_LOOP_27_1: for(int i=0; i<28; i++) {
        VITIS_LOOP_28_2: for(int j=0; j<28; j++) {
#pragma HLS PIPELINE
 in_stream.read(temp_in);
            converter.i = temp_in.data;
            img_buffer[i][j] = converter.f;
        }
    }
    ...
    VITIS_LOOP_40_3: for(int i=0; i<10; i++) {
#pragma HLS PIPELINE
 converter.f = output_buffer[i];
        temp_out.data = converter.i;
